...
export default function Index({ featuredProducts, categories, newArrivals, auth }) {
    ...
    return (
        <div className="min-h-screen bg-neutral-950 text-slate-100 font-sans">
            ...
            <nav className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-black/70 backdrop-blur-xl">
                <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {isLoggedIn ? (
                            <>
                                <Link href={route('client.cart')} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-slate-200 hover:bg-white/10 transition">
                                    <ShoppingCart className="h-5 w-5" />
                                </Link>
                                <Link href={route('client.dashboard')} className="hidden rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-slate-200 transition hover:bg-white/10 md:inline-flex">
                                    Minha Conta
                                </Link>
                            </>
                        ) : (
            ...
            <main className="pt-28">
                    ...
                    <div className="mx-auto max-w-7xl">
                        ...
                        <div className="mt-8 grid gap-5 sm:grid-cols-3">
                            {categories.map((category) => (
                                <div key={category.id} className="group rounded-[1.75rem] border border-white/10 bg-white/5 p-8 transition hover:border-gold-400 hover:bg-white/10">
                                    ...
                                    <p className="mt-2 text-xs text-gold-200">{category.products_count} produtos</p>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <section id="novidades" className="px-4 pb-20 sm:px-6 lg:px-8">
                    <div className="mx-auto max-w-7xl">
                        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                            <div>
                                <p className="text-sm uppercase tracking-[0.32em] text-gold-200">Novidades</p>
                                <h2 className="mt-3 text-3xl font-semibold text-white sm:text-4xl">Chegadas recentes</h2>
                            </div>
                            <p className="max-w-xl text-sm text-slate-400">As últimas peças adicionadas à nossa coleção.</p>
                        </div>
                        ...
                        <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                            {newArrivals.map((product, index) => (
                                <motion.article
                                    ...
                                    className="group overflow-hidden rounded-[1.5rem] border border-white/10 bg-neutral-900/80 p-4 shadow-2xl shadow-black/30 transition hover:-translate-y-1 hover:border-gold-400"
                                >
                                    <div className="relative overflow-hidden rounded-[1.25rem] bg-white/5 p-3">
                                        <div className="aspect-[4/5] overflow-hidden rounded-[1rem] bg-gradient-to-br from-white/10 via-white/5 to-white/0">
                                            {product.image ? (
                                        ...
                                        <div className="absolute left-3 top-3 rounded-full border border-gold-500/30 bg-black/60 px-2 py-1 text-xs uppercase tracking-[0.24em] text-gold-200 shadow-lg shadow-black/40">
                                            Novo
                                        </div>
                                    </div>
                                    <div className="mt-4">
                                        <h3 className="text-lg font-semibold text-white line-clamp-1">{product.name}</h3>
                                        <p className="mt-1 text-xs text-gold-200">{product.category?.name}</p>
                                        <div className="mt-2 flex items-center justify-between">
                                            <span className="text-sm font-semibold text-gold-300">R$ {parseFloat(product.price).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                            <button className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-gold-500 text-neutral-950 transition hover:bg-gold-400">
                                ...
                                featuredProducts.map((product, index) => (
                                    ...
                                    >
                                        ...
                                        <div className="mt-6 flex items-center justify-between gap-4">
                                            ...
                                            <span className="text-lg font-semibold text-gold-300">R$ {parseFloat(product.price).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                        </div>
